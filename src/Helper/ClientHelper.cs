...
public static class ClientHelper
{
    ...
    {
        return container.ExecuteWithRetries(x => x.CreateItemAsync(document, partitionKey, requestOptions, cancellationToken));
    }
    ...
    {
        return container.ExecuteWithRetries(x => x.ReadItemAsync<T>(id, partitionKey, requestOptions, cancellationToken));
    }
    ...
    {
        return container.ExecuteWithRetries(x => x.UpsertItemAsync(document, partitionKey, requestOptions, cancellationToken));
    }
    ...
    {
        return container.ExecuteWithRetries(x => x.DeleteItemAsync<T>(id, partitionKey, requestOptions, cancellationToken));
    }
    ...
    /// <param name="partitionKey"></param>
    /// <returns></returns>
    public static Task<ItemResponse<T>> ReplaceItemWithRetriesAsync<T>(this Container container, T document, string id, PartitionKey partitionKey, ItemRequestOptions? requestOptions = null,
        CancellationToken cancellationToken = default)
    {
        return container.ExecuteWithRetries(x => x.ReplaceItemAsync(document, id, partitionKey, requestOptions, cancellationToken));
    }
    ...
    public static Task<ItemResponse<T>> PatchItemWithRetriesAsync<T>(this Container container, string id, PartitionKey partitionKey, IReadOnlyList<PatchOperation> patchOperations,
        ...
        CancellationToken cancellationToken = default)
    {
        return container.ExecuteWithRetries(x => x.PatchItemAsync<T>(id, partitionKey, patchOperations, patchItemRequestOptions, cancellationToken));
    }
    ...
    private static async Task<T> ExecuteWithRetries<T>(this Container container, Func<Container, Task<T>> function)
    {
        ...
        while (true)
        {
            ...
            finally
            {
                ...
                {
                    logger.Info($"Status [429] received. Will wait for [{timeSpan.Value.TotalSeconds}] seconds.");
