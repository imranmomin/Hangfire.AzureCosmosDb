...
public class CosmosDbConnectionFacts : IClassFixture<ContainerFixture>
{
    ...
    public void CreateExpiredJob_CreatesAJobInTheStorage_AndSetsItsParameters()
    {
        ...
        Dictionary<string, string> parameters = new() { { "Key1", "Value1" }, { "Key2", "Value2" }, { "Key3", "Value3" } };
    ...
    public void GetStateData_ReturnsNull_WhenThereIsNoSuchStateId()
    {
        ...
        // arrange
        IStorageConnection connection = Storage.GetConnection();
        Documents.Job entityJob = new() { CreatedOn = DateTime.UtcNow, StateId = Guid.NewGuid().ToString() };
    ...
    public void SetParameter_UpdatesValue_WhenParameterWithTheGivenName_AlreadyExists()
    {
        ...
        IStorageConnection connection = Storage.GetConnection();
        Job job = Job.FromExpression(() => SampleMethod("wrong"));
        Dictionary<string, string> parameters = new() { { "name", "value" }, { "name1", "value1" }, { "name2", "value2" } };
    ...
    public void RemoveServer_RemovesAServerRecord()
    {
        ...
        QueryRequestOptions queryRequestOptions = new() { PartitionKey = PartitionKeys.Server };
    ...
    public void Heartbeat_UpdatesLastHeartbeat_OfTheServerWithGivenId()
    {
        ...
        // arrange
        List<Documents.Server> servers = new() { new Documents.Server { Id = "server1", CreatedOn = DateTime.UtcNow, LastHeartbeat = new DateTime(2021, 1, 1) }, new Documents.Server { Id = "server2", CreatedOn = DateTime.UtcNow, LastHeartbeat = new DateTime(2021, 1, 1) } };
    ...
    public void RemoveTimedOutServers_DoItsWorkPerfectly()
    {
        ...
        // arrange
        List<Documents.Server> servers = new() { new Documents.Server { Id = "server1", CreatedOn = DateTime.UtcNow, LastHeartbeat = DateTime.UtcNow.AddDays(-1) }, new Documents.Server { Id = "server2", CreatedOn = DateTime.UtcNow, LastHeartbeat = DateTime.UtcNow.AddHours(-12) } };
        Data<Documents.Server> data = new(servers);
        Storage.Container.ExecuteUpsertDocuments(data, PartitionKeys.Server);
    ...
    public void SetRangeInHash_MergesAllRecords()
    {
        ...
        // arrange
        IStorageConnection connection = Storage.GetConnection();
        connection.SetRangeInHash("some-hash", new Dictionary<string, string> { { "Key1", "Value1" }, { "Key2", "Value2" } });
    ...
    public void SetRangeInHash_CanCreateFieldsWithNullValues()
    {
        ...
        // act
        connection.SetRangeInHash("some-hash", new Dictionary<string, string?> { { "Key1", null }, { "Key2", "Value2" } });
    ...
    public void SetRangeInHash_Handles_DistributedLock()
    {
        ...
        List<Hash> hashes = new() { new Hash { Key = "some-hash", Field = "key1", Value = "value1" } };
        Data<Hash> data = new(hashes);
        Storage.Container.ExecuteUpsertDocuments(data, PartitionKeys.Hash);
    ...
    public void GetListCount_ReturnsTheNumberOfListElements()
    {
        ...
        // arrange
        List<List> lists = new() { new List { Key = "list-1", CreatedOn = DateTime.UtcNow }, new List { Key = "list-1", CreatedOn = DateTime.UtcNow } };
    ...
    public void GetListTtl_ReturnsExpirationTimeForList()
    {
        ...
        // arrange
        List<List> lists = new() { new List { Key = "list-1", CreatedOn = DateTime.UtcNow, ExpireOn = DateTime.UtcNow.AddHours(1) }, new List { Key = "list-1", CreatedOn = DateTime.UtcNow } };
        Data<List> data = new(lists);
        Storage.Container.ExecuteUpsertDocuments(data, PartitionKeys.List);
    ...
    public void GetValueFromHash_ReturnsValue_OfAGivenField()
    {
        ...
        // arrange
        List<Hash> hashes = new() { new Hash { Key = "some-hash", Field = "key1", Value = "value1" }, new Hash { Key = "some-hash", Field = "key2", Value = "value2" }, new Hash { Key = "some-other-hash", Field = "key1", Value = "value1" } };
        Data<Hash> data = new(hashes);
        Storage.Container.ExecuteUpsertDocuments(data, PartitionKeys.Hash);
    ...
    public void FetchNextJob_DelegatesItsExecution_ToTheQueue(string queue, string jobId)
    {
        ...
        //act
        IStorageConnection connection = Storage.GetConnection();
        FetchedJob job = (FetchedJob)connection.FetchNextJob(new[]
        {
            queue
